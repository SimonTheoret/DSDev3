...
def bin_counts(df: pd.DataFrame, bins: list, bin_centers: list) -> pd.DataFrame:
    ...
    hist = pd.DataFrame(
        pd.cut(df["rating"], bins=bins, include_lowest=True, labels=bin_centers)
    )
    hist = pd.DataFrame(hist["rating"].value_counts()).reset_index()

    # TODO: Renommer les colonnes, assurez qu'ils ont encore du sens (nous voulons 'rating', 'count')
    # Note: Vous ne devriez pas avoir à renommer les colonnes. Si vous devez,
    # vous avez peut-être une version vielle de Pandas ce qui pourrait amener à
    # des problèmes avec le autograder.

    # TODO: Ajoutez la colonne 'count_norm'
    hist["count_norm"] = hist["count"] / len(df)
...
class PermutationTest:
    def __init__(self, df: pd.DataFrame, n_overrep: int, n_underrep: int):
        ...
        Arguments :
            df (pd.DataFrame) : trame de données complète à partitionner
            (c'est-à-dire inclut les deux groupes)

            n_overrep (entier) : nombre d'éléments dans le groupe surreprésenté

            n_underrep (entier) : nombre d'éléments dans le groupe
            sous-représenté

        n_overrep + n_underrep devrait être == len(df) ! Techniquement < len(df)
        est correct aussi...
        """
    ...
    def job(self, seed: int = None) -> Tuple[int, int]:
        """Échantillonne deux groupes de taille n_overrep, n_underrep et renvoie
        la note maximale pour chacun des groupe dans cet ordre (overrep,
        underrep)
        ...
        Retour:
            Tuple[int, int] : les notes maximales pour chacun des deux groupes,
            dans l'ordre (max(surreprésentation), max(sous-représentation))
        """
        if seed is not None:
            np.random.seed(seed)

        # TODO : échantillonnez deux groupes de taille n_overrep, n_underrep et
        # renvoyez la note maximale pour chacun des groupe dans l'ordre
        # (overrep, underrep)
        ratings = self.df["rating"].to_numpy()
        ratings = np.random.permutation(ratings)
        big_max = max(ratings[0 : self.n_overrep])
        small_max = max(ratings[self.n_overrep : self.n_overrep + self.n_underrep])

        # Astuce : pensez à utiliser np.random.permutation sur un choix
        # approprié de tableau
...
def sample_two_groups(
    ...
    """Exécutez des tests de permutation n_iter sur df, divisé en deux groupes
    (échantillonnés SANS remplacement) de taille n_overrep et n_underrep.
    Renvoyez deux tableaux numpy de longueur n_iter, ou les éléments
    correspondent à la note maximale dans les groupes surreprésentés et
    sous-représentés respectivement.
    ...
    Arguments :
        ...
        n_underrep (entier) : nombre d'échantillons pour le groupe
        sous-représenté

        n_iter (entier, facultatif) : le nombre total d'itérations à exécuter

    Retour:
        Tuple[np.array, np.array] : Deux tableaux contenant les maximums pour le
        groupe surreprésenté et le groupe sous-représenté pour chacune des
        expériences n_iter.
    """
    best_over = []
    best_under = []
    permu = PermutationTest(df, n_overrep, n_underrep)
    for i in tqdm(range(n_iter)):
        big_max, small_max = permu.job()
        best_over.append(big_max)
        best_under.append(small_max)


    # TODO : exécutez n_iter runs de cette expérience et renvoyez un tableau
    # numpy contenant les valeurs maximales des groupes surreprésentés et
    # sous-représentés respectivement.

    # Astuce : enveloppez votre itérateur avec tqdm pour obtenir une barre de
